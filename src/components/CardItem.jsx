import React from 'react';
import { quotes } from "../assets";

const CardItem = ({ grupo, isLoading }) => {
    return (
        <div className="flex justify-between flex-col px-10 py-12 rounded-[20px] max-w-[370px] md:mr-10 sm:mr-5 mr-0 my-5 feedback-card">
            {isLoading ? (
                <>
                    <div className="w-[42.6px] h-[27.6px] bg-gray-300 rounded-lg mb-10"></div>
                    <div className="font-poppins font-normal text-[18px] leading-[32.4px] text-white my-10">
                        <div className="skeleton bg-gray-300 h-6 w-full mb-2"></div>
                        <div className="skeleton bg-gray-300 h-6 w-full mb-2"></div>
                        <div className="skeleton bg-gray-300 h-6 w-full mb-2"></div>
                        <div className="skeleton bg-gray-300 h-6 w-full mb-2"></div>
                    </div>
                    <div className="flex flex-row">
                        <div className="w-[48px] h-[48px] bg-gray-300 rounded-full"></div>
                        <div className="flex flex-col ml-4">
                            <div className="font-poppins font-semibold text-[20px] leading-[32px] text-white">
                                <div className="skeleton bg-gray-300 h-8 w-40"></div>
                            </div>
                            <div className="font-poppins font-normal text-[16px] leading-[24px] text-dimWhite">
                                <div className="skeleton bg-gray-300 h-6 w-40"></div>
                            </div>
                        </div>
                    </div>
                </>
            ) : (
                <>
                    <img src={quotes} alt="double_quotes" className="w-[42.6px] h-[27.6px] object-contain" />
                    <p className="font-poppins font-normal text-[18px] leading-[32.4px] text-white my-10">
                        {grupo?.description}
                    </p>
                    <div className="flex flex-row">
                        <img src={grupo?.img} alt={grupo?.title} className="w-[48px] h-[48px] rounded-full" />
                        <div className="flex flex-col ml-4">
                            <h4 className="font-poppins font-semibold text-[20px] leading-[32px] text-white">
                                {grupo?.title}
                            </h4>
                            <p className="font-poppins font-normal text-[16px] leading-[24px] text-dimWhite">
                                Ver mas
                            </p>
                        </div>
                    </div>
                </>
            )}
        </div>
    );
};

export default CardItem;
